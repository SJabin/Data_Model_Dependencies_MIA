...
def get_fairness(X,y,pred_y, test_x=None, test_y=None, test_pred_y=None):
    ...
    #fairness measure over five features
    for i in range(0, 5):
        temp=temp[temp.iloc[:, i]<=.5]
    
    #records of interest grouped into g; rest of them are in _g 
    gid=list(temp.index)
    ...
    #count the records for y = pred_y = [0,1] in group g
    count1=0
    count0=0 
    if len(gid)!=0: 
        g_y=y[gid]
        ...
        for i in range(0,len(g_y)):
            if g_pred_y[i]==1: count1+=1
            else: count0+=1
    
    #count the records for y = pred_y = [0,1] in group _g   
    _count1=0
    _count0=0
    if len(_gid)!=0: 
        _g_y=y[_gid]
    ...
    # Pr[pred_y=1] and Pr[pred_y=0] in group g
    g_prob1 = 0 if count1==0 else count1/len(gid)
    g_prob0 = 0 if count0==0 else count0/len(gid)
    
    # Pr[pred_y=1] and Pr[pred_y=0] in group g        
    _g_prob1 = 0 if _count1==0 else _count1/len(_gid)
    _g_prob0 = 0 if _count0==0 else _count0/len(_gid)
    
    iy_prob=max(g_prob1,g_prob0)
    _iy_prob=max(_g_prob1,_g_prob0)    
    y_diff=abs(iy_prob-_iy_prob)
    

    g_fair=abs(g_prob1-_g_prob1)
...
def get_mia_indistinguishability(pred_y, test_pred_y):
